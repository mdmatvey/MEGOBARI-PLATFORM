...
import { Container, FormGroup, FormControlLabel, Switch, ThemeProvider, Typography } from "@mui/material";
...
const VectorMap = observer(() => {
  ...
  const [backMarginLeft, setBackMarginLeft] = useState('0')

  useEffect(() => {
    if (userStore.userWidth < 817) {
      setBackMarginLeft('0%')
    } else if (userStore.userWidth < 850) {
      setBackMarginLeft('5%')
    } else if (userStore.userWidth < 900) {
      setBackMarginLeft('10%')
    } else if (userStore.userWidth < 1000) {
      setBackMarginLeft('15%')
    } else if (userStore.userWidth >= 1000) {
      setBackMarginLeft('20%')
    } 
  }, [userStore.userWidth])
  ...
  return (
    <ThemeProvider theme={theme}>
      <Container>
        <Typography variant="h2" component="h1" fontWeight={'bold'}>MEGOBARI</Typography>
        {
          userStore.userWidth < 550 
          ?
            <RotateScreen />
          :
            <div className="back" style={{ position: 'relative', marginLeft: backMarginLeft, overflow: 'scroll' }}> 
              <div style={{ opacity: isSatellite ? 1 : 0, transition: '.2s', position: 'relative', top: 40, left: -25, width: 710, height: 525 }}>
                <img 
                  src={SatelliteShot} 
                  alt="Satellite shot" 
